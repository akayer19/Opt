...
class Lineup:
    ...
    def __repr__(self):
        return 'Lineup: projection %s, budget %s, Total Ownership %s' % (self.fantasy_points_projection, self.salary_costs, self.ownerships)
    ...
    @property
    def ownerships(self) -> float:
        return round(sum(player.projected_ownership or player.projected_ownership for player in self.players), 4)
